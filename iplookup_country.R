...
ggplot( data = cnt_top, aes(x = cnt_top$country, y = cnt_top$freq) ) + geom_bar(stat = 'identity', fill = cnt_top$freq) + xlab('Country') + ylab('Count') + ggtitle('Top 10 Request from Country') + theme(axis.text.x = element_text(color = "black", size = 8, angle = 45),  axis.text.y = element_text(color = "black", size=8, angle=45))
